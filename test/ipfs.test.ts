...
describe('ipfs', function () {
  ...
  it('should save file with correct ipfs hash', async function () {
  ...
  it('should save object with correct ipld hash', async function () {
    this.timeout(80 * 1000);
  ...
  it('should resolve object props', async function () {
    this.timeout(80 * 1000);
  ...
  it('should resolve trie props', async function () {
    this.timeout(80 * 1000);
  ...
  it('should build deterministic user manifest hash', async function () {
    this.timeout(80 * 1000);
  ...
  it('should correctly convert peerId to string representations', async function () {
    this.timeout(80 * 1000);
    ...
    expect(privateBase64.indexOf('CAAS')).to.equals(0);
    expect(privateBase64.length).to.be.greaterThan(1500);

    expect(publicBase64.indexOf('CAAS')).to.equals(0);
    expect(publicBase64.length).to.equals(400);

    const pubKey = peerIdHelper.base64ToPublicKey(publicBase64);
    ...
    expect(peerIdHelper.publicKeyToBase64(pubKey)).to.equals(publicBase64);
  ...
  it('should save directory correctly', async function () {
    this.timeout(80 * 1000);
    ...
    const list = await node.nodeLs(res.id);
    expect(res.id).to.be.a('string');
    expect(list.map(item => item.name).sort()).to.deep.equal(['file1.txt', 'file2.txt']);
  });
});
